...
class StaticGroupedAssigner(Assigner):
    ...
    GroupedAssigner -        Given task groups and a list of collaborators that belong to
                             that task group, carry out tasks for each round of experiment
    """
    ...
    def define_task_assignments(self):
        ...
        """
        assert (sum([len(group['collaborators']) for group in self.task_groups]) == len(self.authorized_cols)
                and set([col for group in self.task_groups for col in group['collaborators']]) == set(
