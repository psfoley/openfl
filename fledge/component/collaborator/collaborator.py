...
class Collaborator(object):
    ...
    Args:
        ...
        model: The model
        opt_treatment (string): The optimizer state treatment (Defaults to "CONTINUE_GLOBAL",
        ...
        delta_updates (bool): True = Only model delta gets sent. False = Whole model
        ...
        while True:
            ...
            if tasks.quit:
                ...
                for task in tasks.tasks:
                    self.do_task(task, tasks.round_number)
                self.logger.info(
                    'All tasks completed on {} for round {}...'.format(self.collaborator_name, tasks.round_number))
                break
    ...
    def get_data_for_tensorkey(self, tensor_key):
        ...
        if nparray is None:
            if origin == self.collaborator_name:
                self.logger.info(
                    'Attempting to find locally stored {} tensor from prior round...'.format(tensor_name))
                ...
                while prior_round >= 0:
                    ...
                    if nparray is not None:
                        self.logger.debug(
                            'Found tensor {} in local TensorDB for round {}'.format(tensor_name, prior_round))
                        return nparray
                    prior_round -= 1
            ...
            if len(tensor_dependencies) > 0:
                ...
                if prior_model_layer is not None:
                    ...
                    new_model_tk, nparray = self.tensor_codec.apply_delta(tensor_dependencies[1], uncompressed_delta,
                                                                          prior_model_layer)
    ...
    def get_aggregated_tensor_from_aggregator(self, tensor_key, require_lossless=False):
        ...
        If the key specifies an uncompressed tensor (or just omits a compressed tag),
        the decompression operation will be skipped

        Args
        ----
        tensor_key  :               The requested tensor
        permit_lossy_compression:   Should compression of the tensor be allowed in flight?
                                    For the initial model, it may affect convergence to apply lossy
                                    compression. And metrics shouldn't be compressed either
    ...
    def send_task_results(self, tensor_dict, round_number, task_name):
        ...
        for tensor in tensor_dict:
            ...
            if report:
                self.logger.info(
                    f'Sending metric for task {task_name}, round number {round_number}:'
    ...
    def nparray_to_named_tensor(self, tensor_key, nparray):
        ...
        if 'trained' in tags and self.delta_updates:
            ...
            if model_nparray is not None:
                ...
                delta_comp_tensor_key, delta_comp_nparray, metadata = self.tensor_codec.compress(delta_tensor_key,
                                                                                                 delta_nparray)
    ...
    def named_tensor_to_nparray(self, named_tensor):
        ...
        tensor_key = TensorKey(named_tensor.name, self.collaborator_name, named_tensor.round_number,
                               named_tensor.report, tuple(named_tensor.tags))
        tensor_name, origin, round_number, report, tags = tensor_key
