...
class TensorDB(object):
    ...
    def __repr__(self):
        with pd.option_context('display.max_rows', None):
            return 'TensorDB contents:\n{}'.format(
                self.tensor_db[['tensor_name', 'origin', 'round', 'report', 'tags']])
    ...
    def get_aggregated_tensor(self, tensor_key, collaborator_weight_dict, aggregation_functions):
        """
        Determines whether all of the collaborator tensors are present for a given
        tensor key, and returns their weighted average

        Args:
            tensor_key: The tensor key to be resolved. If origin 'agg_uuid' is present,
                                        can be returned directly. Otherwise must compute
                                        weighted average of all collaborators
            collaborator_weight_dict:	List of collaborator names in federation and their respective weights
            aggregation_functions:      Call the underlying numpy aggregation function. Default
                                        is just the weighted average.
                                        ONLY THE FIRST FUNCTION will get recorded as the aggregated tensor
        ...
        for aggregation_function in aggregation_functions:
            if aggregation_function == 'weighted_average':
                agg_nparray = np.average(concat_nparray, weights=np.array(list(collaborator_weight_dict.values())),
                                         axis=0)
            else:
                ...
                if callable(agg_func):
                    ...
                    raise KeyError('{} is not a valid numpy function'.format(aggregation_function))

            # TODO This enforces that only the first aggregation function will be associated with
