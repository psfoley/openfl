...
class FederatedModel(TaskRunner):
    ...
    Args:
        ...
        optimizer : lambda function (only required for pytorch)
            The optimizer should be definied within a lambda function. This allows the optimizer
            to be attached to the federated models spawned for each collaborator.
    ...
    def __getattribute__(self, attr):
        ...
        if attr in ['reset_opt_vars', 'initialize_globals', 'set_tensor_dict', 'get_tensor_dict',
                    ...
                    'save_native', 'load_native', 'rebuild_model', 'set_optimizer_treatment',
    ...
    def setup(self, num_collaborators):
        ...
        Returns:
            List of models"""
        return [
            FederatedModel(self.build_model, optimizer=self.lambda_opt, loss_fn=self.loss_fn, data_loader=data_slice)
            for data_slice in self.data_loader.split(num_collaborators, equally=True)]
