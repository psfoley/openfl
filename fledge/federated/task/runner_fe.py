...
class FastEstimatorTaskRunner(TaskRunner):
    def __init__(self, estimator, **kwargs):
        ...
        for k, v in estimator.system.__dict__.items():
            ...
            if k == 'traces':
                ...
                estimator_kwargs[k] = v + [
                    ProgressLoader(lambda: {'epoch_idx': self.epoch_idx, 'global_step': self.global_step})]
    ...
    def train(self, col_name, round_num, input_tensor_dict, epochs, **kwargs):
        ...
        tags = ('trained',)
        output_metric_dict = {
            TensorKey(metric, origin, round_num, True, ('metric',)): np.array(
                list(summary.history['train'][metric].values())[-1]) for metric in param_metrics}
    ...
    def load_native(self, filepath, **kwargs):
        ...
        Args:
            filepath (string): Path to frame-work specific file to load. For frameworks that
             use multiple files, this string must be used to derive the other filepaths.
