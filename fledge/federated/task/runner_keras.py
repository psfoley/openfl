...
class KerasTaskRunner(TaskRunner):
    ...
    def train(self, col_name, round_num, input_tensor_dict, epochs, **kwargs):
      ...
      # TODO Currently assuming that all metrics are defined at initialization (build_model). If metrics are added (i.e. not a subset of what was originally defined) then the model must be recompiled. 
      ...
      # TODO if there are new metrics in the flplan that were not included in the originally compiled model, that behavior is not currently handled. 
        for param in param_metrics:
            if  param not in model_metrics_names:
                error = 'KerasTaskRunner does not support specifying new metrics. Param_metrics = {}, model_metrics_names = {}'.format(param_metrics,model_metrics_names)
