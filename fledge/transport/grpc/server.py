  ...
  ,              agg_port,
    ...
    def validate_collaborator(self, request, context):
        ...
        if  not self.disable_tls:
            ...
            if not self.aggregator.valid_collaborator_CN_and_id(common_name, collaborator_common_name):
                raise ValueError(f'Invalid collaborator. CN: |{common_name}| collaborator_common_name: |{collaborator_common_name}|')
    ...
    def serve(self):
        ...
        if  self.disable_tls:
            ...
            with open(self.private_key, 'rb') as f: private_key       = f.read()
            with open(self.certificate, 'rb') as f: certificate_chain = f.read()
            with open(self.ca,          'rb') as f: root_certificates = f.read()
