...
class Aggregator(object):
    ...
    def handle_local_model_update(self, message):
        ...
        for i in range(len(self.model.tensors)):
            ...
            # local collaborator tensor
            l = model_proto.tensors[i]
            ...
            self.model.tensors[i].values = np.average([g.values, l.values], weights=[weight_g, weight_l])

        # store the loss results and training update size
        self.loss_results[message.header.sender] = message.loss
        self.collaborator_training_sizes = message.data_size
    ...
    def handle_local_validation_results(self, message):
        ...
        self.collaborator_validation_sizes = message.data_size
    ...
    def handle_job_request(self, message):
        ...
        elif message.sender not in self.collaborator_validation_sizes:
            job = JOB_VALIDATE
        # else, check if this collaborator has not sent training results
        elif message.sender not in self.collaborator_training_sizes:
            job = JOB_TRAIN
    ...
    def create_reply_header(self, message):
        return MessageHeader(sender=self.id, recipient=message.sender, federation_id=self.fed_id, counter=message.counter)
