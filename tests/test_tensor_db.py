...
def test_tensor_db_multiple_insert():
    tensor_db = TensorDB()
    tensor_key = TensorKey('conv1','col_1',0,False,('model',))
    tensor_key2 = TensorKey('conv1','col_1',0,False,('trained',))
...
def test_tensor_db_aggregation():
    tensor_db = TensorDB()
    tensor_key = TensorKey('conv1','agg',0,False,('model','col_1'))
    nparray = np.arange(10)
    tensor_key_dict = {tensor_key: nparray}
    tensor_db.cache_tensor(tensor_key_dict)
    tensor_key2 = TensorKey('conv1','agg',0,False,('model','col_2'))
    nparray2 = np.arange(10)*3
    tensor_key_dict2 = {tensor_key2: nparray2}
    tensor_db.cache_tensor(tensor_key_dict2)
    #Lookup tensorkey
    lookup_tk = TensorKey('conv1','agg',0,False,('model',))
