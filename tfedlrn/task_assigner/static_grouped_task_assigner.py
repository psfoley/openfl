...
class StaticGroupedTaskAssigner(TaskAssigner):
    """
    ...
    StaticGroupedTaskAssigner  - Given a set of task groups, and a list of collaborators for that group, assign tasks for 
    ...
    def define_task_assignments(self):
        ...
        """
        assert(sum([len(group['collaborators']) for group in self.task_groups]) == len(self.collaborator_list) and \
                set([col for group in self.task_groups for col in group['collaborators']]) == set(self.collaborator_list)), \
                'Collaborators in each group must be distinct: {}, {}'.format(set([col for group in self.task_groups for col in group['collaborators']]), set(self.collaborator_list))
