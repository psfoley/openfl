...
class TensorCodec(object):
    ...
    def compress(self,tensor_key,data,require_lossless=False,**kwargs):
        ...
        if lossless:
            compressed_nparray, metadata = self.lossless_pipeline.forward(nparray,**kwargs)
        else:
            compressed_nparray, metadata = self.compression_pipeline.forward(nparray,**kwargs)
        #Define the compressed tensorkey that should be returned ('trained.delta'->'trained.delta.lossy_compressed')
        if not self.compression_pipeline.is_lossy() or lossless:
    ...
    def decompress(self,tensor_key, data, transformer_metadata, require_lossless=False,**kwargs):
        ...
        if lossless:
            decompressed_nparray = self.lossless_pipeline.backward(nparray,metadata,**kwargs)
        else:
            decompressed_nparray = self.compression_pipeline.backward(nparray,metadata,**kwargs)
    ...
    def find_dependencies(self,tensor_key,send_model_deltas):
        ...
        if 'model' in tensor_key[3] and send_model_delta:
            if tensor_key[2] >= 1:
